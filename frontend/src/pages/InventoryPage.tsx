...
export function InventoryPage() {
  ...
  async function handleSave(
    ...
    const draft = drafts[balance.product_id]
    ...
    setSavingProductId(balance.product_id)
    setErrorMessage('')
    setSuccessMessage('')
    ...
    try {
      const updatedBalance =
        await updateInventoryBalance(
          balance.product_id,
          {
            program_quantity: programQuantity,
  ...
  const totalLosses = balances.reduce(
    (total, balance) => {
      const draft = drafts[balance.product_id]
